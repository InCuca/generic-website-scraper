...
const scrape = async (allSG, scraper) => {
  ...
  const promise = new Promise(() => {
    elements.forEach(async (element) => {
      const card = await page.evaluate((el) => el.innerHTML, element);
      ...
      const stars = $('span:contains("stars")').parent().text().replace('stars', '')
        .trim() || null;
      const forks = $('span:contains("forks")').parent().text().replace('forks', '')
        .trim() || null;
      const issues = $('span:contains("issues")').parent().text().replace('issues', '')
        .trim() || null;
